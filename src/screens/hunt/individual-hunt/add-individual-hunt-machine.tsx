...
export const addIndividualHuntMachine = createMachine(
    {
        id: "addHunt",
        schema: {
            events: {} as
    ...
    },
    {
        ...
        services: {
            addHunt: (context) => async (send) => {
                ...
                try {
                    const result = await api.addIndividualHunt(context.payload);
                    if (result.success) {
...
type AddIndividualHuntStatusDialogProps = {
    isEditing?: boolean;
    service: ActorRefFrom<typeof addIndividualHuntMachine>;
};

export function AddIndividualHuntStatusDialog({ service, isEditing }: AddIndividualHuntStatusDialogProps) {
    ...
    React.useEffect(() => {
        const subscription = service.subscribe((state) => {
            const message = "AIH " + JSON.stringify(state.value) + " " + JSON.stringify(state.event);
            logger.log(message);
        });
