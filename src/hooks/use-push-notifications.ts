...
export function usePushNotifications() {
    ...
    React.useEffect(() => {
        ...
        async function registerForPushNotifications() {
            ...
            const newToken: PushNotificationsToken = { token, language, tokenEnv: ENV.NOTIFICATIONS_ENVIRONMENT };
            if (
                oldToken &&
                oldToken.token === newToken.token &&
                oldToken.language === newToken.language &&
                oldToken.tokenEnv === newToken.tokenEnv
            ) {
                return;
            }
    ...
    React.useEffect(() => {
        ...
        async function onNotificationOpened(notification: Notifications.NotificationContent) {
            ...
            switch (data.type) {
                case "news": {
                    await queryClient.refetchQueries({ queryKey: queryKeys.news });
                ...
                case "dh_created": {
                    if (data.sourceId) {
                        await queryClient.refetchQueries({ queryKey: queryKeys.hunts });
